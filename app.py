...
# 💡 Prevent PyTorch from freezing single-core Streamlit Cloud CPUs
...
# ===================================================
# 3. Load Trained Pipeline Safely (Cached)
# ===================================================
...
if pipeline is not None:
    ...
    distance_km = st.sidebar.number_input("Distance (km)", min_value=500.0, max_value=15000.0, value=7750.0, step=1.0)
    weight_mt = st.sidebar.number_input("Weight (MT)", min_value=1.0, max_value=500.0, value=243.5, step=1.0)
    fuel_index = st.sidebar.number_input("Fuel Price Index", min_value=1.00, max_value=5.00, value=2.84, step=0.01)
    ...
    if st.sidebar.button("Run Predictions"):
        ...
        st.sidebar.metric("⏰ Estimated Lead Time", f"{days}d {hours}h ({pred_days_float:.1f} Days)")
